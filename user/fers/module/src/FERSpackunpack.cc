...
//      SpectEvent_t tmpEvent = FERSunpack_spectevent(vec);
//      //SpectEvent_t* Event0 = &tmpEvent;
//      //Event = (void)tmpEvent;
...
SpectEvent_t FERSunpack_spectevent(std::vector<uint8_t> *vec)
{
  ...
  //EUDAQ_WARN("* index: "+std::to_string(index)+" read uint16 energyLG[64] ");
  for (int i=0; i<nchan; ++i) {
  ...
  return tmpEvent;
  //Event = tmpEvent;
  //EUDAQ_WARN("* Event tstamp_us: "+std::to_string( ((SpectEvent_t*)Event)->tstamp_us));
}
